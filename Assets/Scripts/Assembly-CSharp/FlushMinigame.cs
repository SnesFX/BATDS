...
public class FlushMinigame : MonoBehaviour
{
	...
	private Interactable_Toilet toiletRef;
	...
	private void OnEnable()
	{
		...
		Marker.rotation = Quaternion.Euler(0f, 0f, Random.Range(0, 180));
		if ((bool)Audio)
		{
			Audio.PlayClip(0);
		}
		DialRotation = 1f;
		Dial.eulerAngles = Vector3.zero;
	...
	private void Update()
	{
		...
		if (!stopDial)
		{
			if (Dial.eulerAngles.z > 180f || Dial.eulerAngles.z < 0f)
			{
				DialRotation *= -1f;
			}
			Dial.Rotate(0f, 0f, (150f + (float)multiplier) * Time.deltaTime * DialRotation);
			return;
		}
	...
	public void Activate(Interactable_Toilet toilet)
	{
		toiletRef = toilet;
